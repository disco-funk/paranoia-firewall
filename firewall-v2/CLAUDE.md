...
- **Clock is set in-place, not by aborting:** At startup the script shows the time and, if the operator says it is wrong, reads a `YYYY-MM-DD HH:MM:SS` value and applies it with `timedatectl set-time`. `timedatectl` refuses a manual set while NTP sync is active, so the script runs `timedatectl set-ntp off` first and re-enables it (`set-ntp true`) after DHCP is up. Keep that off/on pairing — dropping the `set-ntp off` makes `set-time` fail. No inline `sudo` (constraint applies): the script is already root.
- **Ethernet connection may be chosen:** Connection detection collects all ethernet connections from `nmcli` into an array. One → used automatically; several → the operator picks via a `select` menu; none → error and exit. Do not collapse this back to a single-value capture — with two or more connections that yields a multi-line string that is not a valid connection name, and every later `nmcli connection modify` fails.
