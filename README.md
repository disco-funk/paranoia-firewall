...
The `config` section also validates DNSSEC end to end: a signed domain must resolve, the bogus-signed `dnssec-failed.org` must be rejected *as a DNSSEC failure*, and a nonexistent name under the signed `iana.org` zone must return NXDOMAIN rather than a validation error. All three matter — without the first, a totally broken resolver would appear to "reject the bogus zone" correctly. (The nonexistent-name case uses `iana.org` rather than `example.com`, which is Cloudflare-hosted and answers nonexistent names with NOERROR/NODATA — *compact denial of existence* — instead of NXDOMAIN.) On the Pi, where `dig` reports a status code rather than prose, the NXDOMAIN-vs-SERVFAIL distinction stands in for the explicit "DNSSEC failure" wording; these functional checks skip cleanly if `dig` (dnsutils) is not installed.
